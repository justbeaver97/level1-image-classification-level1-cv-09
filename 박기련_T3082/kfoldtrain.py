...
def get_class_distribution(dataset):
    count_dict = {i:0 for i in range(18)}
...
def train(data_dir, model_dir, args):
    ...
    # -- augmentation
    transform_module = getattr(import_module("dataset"), args.baseaugmentation)  # default: BaseAugmentation
    ...
    # -- kfold
    labels = [k for v, k in dataset]
    kfold = StratifiedKFold(n_splits=5)
    for fold_idx, (train_idx, valid_idx) in enumerate(kfold.split(dataset, labels)):
        ...
        # -- augmentation
        train_transform_module = getattr(import_module("albumdataset"), args.augmentation)  # default: BaseAugmentation
        ...
        class_counts = [i for i in get_class_distribution(train_set).values()]
        # print(class_counts)
        num_samples = sum(class_counts)
        train_labels = [k for v, k in train_set]
        ...
        for epoch in range(args.epochs):
            ...
            with torch.no_grad():
                ...
                for val_batch in val_loader:
                    ...
                    f1_item = f1_score(outs.argmax(dim=1).cpu(), labels.cpu(), average='macro')
                ...
                if val_f1 > best_val_f1:
                    ...
                    torch.save(model.module.state_dict(), f"{save_dir}/best{fold_idx}.pth")
                    best_val_f1 = val_f1
                torch.save(model.module.state_dict(), f"{save_dir}/last{fold_idx}.pth")
                print(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--baseaugmentation', type=str, default='BaseAugmentation', help='data augmentation type (default: BaseAugmentation)')
    parser.add_argument('--augmentation', type=str, default='Albumentation', help='data augmentation type (default: BaseAugmentation)')
    ...
    parser.add_argument('--model_dir', type=str, default=os.environ.get('SM_MODEL_DIR', './kfoldmodel'))
    ...
    # wandb
    wandb.init(project="image-classification-level1", entity="park95gr")
